      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Every story passes through four specialized agents. Each one has a
            specific job, creating a review loop that catches what single-shot
            generation misses.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {phases.map((phase, i) => (
            <div
              key={phase.name}
              className={`rounded-xl border ${phase.bg} p-5`}
            >
              <div className="flex items-center gap-3 mb-4">
                <span className={`font-mono text-xs ${phase.color}`}>
                  Phase {i + 1}
                </span>
              </div>
              <h3 className={`text-lg font-semibold mb-3 ${phase.color}`}>
                {phase.name}
              </h3>
              <ul className="space-y-2">
                {phase.tasks.map((task) => (
                  ...
                  >
                    <span
                      className={`mt-1.5 w-1.5 h-1.5 rounded-full shrink-0 ${phase.color.replace("text-", "bg-")}`}
                    />
                    {task}
                  </li>
                ))}
